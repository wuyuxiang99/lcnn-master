...
class MultitaskHead(nn.Module):
    def __init__(self, input_channels, num_class):
        ...
        heads = []
        for output_channels in sum(M.head_size, []):  # sum合并M.head_size列表中的所有元素到一个空list中
            heads.append(
                nn.Sequential(
...
class MultitaskLearner(nn.Module):
    def __init__(self, backbone):
        ...
        head_size = M.head_size  # head_size: [[2], [1], [2]]
        self.num_class = sum(sum(head_size, []))  # num_class: 2+1+2 = 5
    ...
    def forward(self, input_dict):
        ...
        # switch to CNHW
        for task in ["jmap"]:
            T[task] = T[task].permute(1, 0, 2, 3)  # permute 维度换位
        for task in ["joff"]:
