...
class LineVectorizer(nn.Module):  # 线向量化工具
    def __init__(self, backbone):
        super().__init__()
        self.backbone = backbone  # 骨干网络backbone

        lambda_ = torch.linspace(0, 1, M.n_pts0)[:, None]  # n_pts0 = 32 <- Np
        self.register_buffer("lambda_", lambda_)
        self.do_static_sampling = M.n_stc_posl + M.n_stc_negl > 0  # 正样本 + 硬负样本 > 0 -> 300 + 40 Ns+ + Ns-
        self.fc1 = nn.Conv2d(256, M.dim_loi, 1)  # dim_loi = 128
        scale_factor = M.n_pts0 // M.n_pts1  # n_pts1 = 8 scale_factor = 4
        if M.use_conv:  # use_conv = 0
            ...
            self.fc2 = nn.Sequential(
                nn.Linear(M.dim_loi * M.n_pts1 + FEATURE_DIM, M.dim_fc),  # 128 * 8 + 8 = 1032
                nn.ReLU(inplace=True),
                nn.Linear(M.dim_fc, M.dim_fc),  # dim_fc = 1024  中间有1024个神经元
                nn.ReLU(inplace=True),
                nn.Linear(M.dim_fc, 1),  # 展平成一维向量
            )
        self.loss = nn.BCEWithLogitsLoss(reduction="none")

    def forward(self, input_dict):
        result = self.backbone(input_dict)  # backbone 输出的特征图
        h = result["preds"]
        ...
        for i, meta in enumerate(input_dict["meta"]):
            p, label, feat, jc = self.sample_lines(  # feature junction
        ...
        f = torch.cat(fs)
        x = x.reshape(-1, M.n_pts1 * M.dim_loi)  # 8 128
        x = torch.cat([x, f], 1)
        ...
        if input_dict["mode"] != "training":
            ...
            result["preds"]["lines"] = torch.cat(lines)  # 来自backbone
